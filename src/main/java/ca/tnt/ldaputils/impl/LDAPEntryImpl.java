...
/**
 * LDAPObject implementing class.  All inheriting classes should implement a
 * getAttributeXAsString() type method, such as getMail().  But they may also
 * need to implement a getAttributeXValues(), such as getMails().  So, it should
 * simply be a plural of the attribute.  If an object had getBusinessCategory()
 * it could also have a getBusinessCategories().  The plural versions should
 * ALWAYS return a "List" for consistancy.
 * <p/>
...
{
    static final Logger logger = Logger.getLogger(LDAPEntryImpl.class);
     ...
     */
    private void loadAttributes(final Attributes attributes) throws NamingException
    {
...
/*    public LDAPObjectImpl(Map attributes) throws NamingException
    ...
    private void verifyObject() throws NamingException
    {
        ...
        {
            logger.fatal(getStringValue("cn") + " has no objectClass?");
        }
        else
        {
            logger.debug(
                getStringValue("cn") + " has objectClasses: " + objectClasses.toString());
        }
    }
...
//        return LdapManager.createInstance(attributes, type, getDn());
    ...
    public void modifyBatchAttribute(final int operation, final String attribute,
        final Object value
    )
    {
